...
class x2vTrainer(object):
    ...
    def build_search_dataset(self):
        ...
        for itera in range(0, iterations):
            if self.network.startswith('Graph'):
                inputs,lable,funcs = self.get_graph_search_inputs(itera,test_dataset)
        ...
        fun_database = os.path.join(self.datacenter, 'I2V_XA_busybox_search_funcs_vec.json')
    ...
    def get_graph_search_inputs(self,itera,dataset):
        ...
        pair = self.get_data.get_graph_singles(funcs)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-b", "--batch_size", type=str, help="model network for batch_size")
