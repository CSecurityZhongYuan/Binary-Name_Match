...
class x2vTrainer(object):
    ...
    def build_search_dataset(self):
        ...
        # best_model=torch.load(model_name).get('val_auc')
        # print(best_model)
        # exit(0)
        ...
        for itera in range(0, iterations):
            if self.network.startswith('acfg'):
                inputs, lable ,funcs = self.get_acfg_inputs(itera, test_dataset)
        ...
        fun_database = os.path.join(self.datacenter, 'Gemini_XA_busybox_search_funcs_vec.json')
    ...
    def get_acfg_inputs(self,itera,dataset):
        ...
        pair = self.get_data.get_acfg_singles(pairs)
